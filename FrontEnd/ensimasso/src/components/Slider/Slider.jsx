...
const Slider = () => {
  ...
  useEffect(() => {
    ...
    const moveSlider = (direction) => {
      ...
      sliderItems.forEach(item => item.classList.remove('fade-in', 'fade-out', 'scale-up'));
    
      const currentThumbnail = thumbnailItems[0].querySelector('img');
      currentThumbnail.classList.add('scale-up');
    
      setTimeout(() => {
        if (direction === 'next') {
          sliderItems[0].classList.add('fade-out');
          sliderItems[1].classList.add('fade-in');
    
          setTimeout(() => {
            sliderListRef.current.appendChild(sliderItems[0]);
            sliderItems[0].classList.remove('fade-out');
          }, 500);
        } else {
          sliderItems[sliderItems.length - 1].classList.add('fade-in');
          sliderItems[sliderItems.length - 2].classList.add('fade-out');
    
          setTimeout(() => {
            sliderListRef.current.prepend(sliderItems[sliderItems.length - 1]);
            sliderItems[sliderItems.length - 2].classList.remove('fade-out');
          }, 500);
        }
    
        // Mettez à jour l'animation ici
        if (direction === 'next') {
          thumbnailRef.current.appendChild(thumbnailItems[0]);
        } else {
        ...
        // Retirer l'effet de scale après un court délai
        setTimeout(() => {
          currentThumbnail.classList.remove('scale-up');
        }, 500);
      }, 0);
    };    
